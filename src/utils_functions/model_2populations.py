...
def _numba_dfun(y, c_pop, x0, Iext, Iext2, a, b, tt, Kvf, c, d, r, r2, Ks, Kf, aa, bb, tau, modification, Istim, n_stim,
                threshold, ydot):
    ...
    # TODO adjust I stim weighting coefficients properly
    # ydot[0] = tt[0] * (y[1] - y[2] + Iext[0] + Istim[0] * 5 + Kvf[0] * c_pop1 + ydot[0] * y[0])
    ydot[0] = tt[0] * (y[1] - y[2] + Iext[0] + Istim[0] * 3 + Kvf[0] * c_pop1 + ydot[0] * y[0])
    ...
    # slope
    ydot[6] = tt[0] * r2[0] * (-0.3 * y[6] + 20 * abs(Istim[0]) + Kf[0] * c_pop3)


class EpileptorStim2Populations(ModelNumbaDfun):
    r"""
    ...
    variables_of_interest = List(
        ...
        choices=('x1', 'y1', 'z', 'x2', 'y2', 'g', 'm', 'x2-x1'),
        default=('x2-x1', 'z'),
        doc="Quantities of the Epileptor available to monitor.",
    )

    state_variables = ('x1', 'y1', 'z', 'x2', 'y2', 'g', 'm')
    _nvar = 7
    ...
    def dfun(self, x, c, local_coupling=0.0, stimulus=0.0):
        ...
        if isinstance(stimulus, numpy.ndarray):  # and stimulus.any() > 0.0:
            ...
            # self.x0 = self.x0 + stimulus[0][0][0]/4

            # if stimulus[0][n_region][0]>0:
            # self.n_stim = self.n_stim + 0.5 # cause dfun is called twice in the integrator, otherwise +1
            #     self.x0 = numpy.array([-2.2]) + self.n_stim/5
            #     self.timer = numpy.array([50000])
        ...
        deriv = _numba_dfun(x_, c_,
                            self.x0, Iext, self.Iext2, self.a, self.b, self.tt, self.Kvf,
                            self.c, self.d, self.r, self.r2, self.Ks, self.Kf, self.aa, self.bb, self.tau,
                            self.modification, self.Istim, self.n_stim, self.threshold)
