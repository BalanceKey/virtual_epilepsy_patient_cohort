...
pid = 'id026_fs'#'id001_bt'#'id002_sd'#'id030_bf'  # 'id005_ft'
...
szr_name = f'{subj_proc_dir}/seeg/fif/FS_interCritique_130917B-EEX_0001.json'
# Load seizure data
...
num_regions = len(roi)
epileptors.slope = np.ones(num_regions) * (0)
epileptors.Iext = np.ones(num_regions) * (3.1)
epileptors.Iext2 = np.ones(num_regions) * (0.45)
epileptors.Ks = np.ones(num_regions) * (1.0) * 0.01
epileptors.Kf = np.ones(num_regions) * (1.0) * 0.001  # ?
epileptors.Kvf = np.ones(num_regions) * (1.0) * 0.01  # ?
...
if clinical_hypothesis:
    ...
    #     ez.remove('Left-Superior-parietal-lobule-P1')
    pz.append('Right-Amygdala')
    pz.append('Right-Parieto-occipital-sulcus')
    pz2.append('Right-Lingual-gyrus')
    pz2.append('Left-Precuneus')
...
epileptors.Iext[index_ez] = 6.0
epileptors.Iext[index_pz] = 6.0
epileptors.Iext[index_pz2] = 6.0
...
if noisy_sim:
    ...
    nsig = np.array([0.8, 0.8, 0., 0.002, 0.002, 0.])
...
init_cond=np.array([-1.46242601e+00, -9.69344913e+00, 6.2, -1.11181819e+00, -9.56105974e-20, -4.38727802e-01])
