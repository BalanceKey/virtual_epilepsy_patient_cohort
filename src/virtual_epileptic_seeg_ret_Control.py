...
pid = 'id006_mr'  # 'id005_ft'#'id003_mg'
...
# # 2. Choose a seizure, just use gain matrix, we will try to model seizures as different from this one
szr_name = f'{subj_proc_dir}/seeg/fif/MRcrise1.json'
# Load seizure data
...
VEC_patient_data = f'{save_data}/../VirtualEpilepticCohort/derivatives/tvb/{pid_bids}/ses-01/{acq}/parameters/'
...
if clinical_hypothesis:
    ...
    pz2 = [ezroi[iez] for iez in np.where((ezval > 0) & (ezval < 0.2))[0]]
    print('EZ: ', ez)
    print('PZ: ', pz)
    print('PZ2: ', pz2)

samples = 23 # TODO change
