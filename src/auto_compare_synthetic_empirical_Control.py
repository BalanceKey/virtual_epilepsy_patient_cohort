...
save_analysis = f'{subjects_dir}/comparison_emp_sim_Control' # To save the stats
...
for ich, ch in enumerate(ch_names):
    if peak_vals[ich] > 1.4: # TODO change
...
plot_signal(t, y, ch_names, seeg_info=seeg_info, datafeature=slp, ez_channels_slp=ez_channels_slp,
            onsets=t[onsets], offsets=t[offsets], save_path=save_path)


#%% Do the same for synthetic data
# clinical_hypothesis = False
database = f'/Users/dollomab/MyProjects/Epinov_trial/simulate_data/patients/BIDS/ControlCohort/'
sim_patient_data = f'{database}/{pid_bids}'
run = 2#szr_index + 1 # TODO change
...
for ich, ch in enumerate(ch_names_sim):
    if peak_vals_sim[ich] > 4.7: # TODO change
...
t_SO_max = seeg_info['onset'] + 13 # TODO see if needs change
...
t_SO_max_sim = min([onsets_grouped_sim[i] for i in np.where(binary_sim == 1)[0]]) + 0.2 # TODO see if needs change
...
plot_signal_ez_pz(t_sim, y_sim_AC, peak_vals_sim, ch_names_sim, EZ_channels_sim, PZ_channels_sim, scaleplt=0.2, save_path=save_path_ezpz_sim)
...
for ch_idx in range(len(ch_names_common)):
    if ez_channels_slp_sim_c[ch_idx] == 1:
        binarized2Dslp_sim[onsets_sim_c[ch_idx]-start:offsets_sim_c[ch_idx]-start, ch_idx] = 1
...
#                            'PCA_correlation_slp', 'PCA1_correlation_slp',
#                            'J_SO_overlap', 'J_SP_overlap', 'J_NS_overlap',
#                            '2D_agreement', '2D_correlation', '2D_mse', '2D_rmse'])
