...
for the Control Cohort of interictal data
'''
import sys
import mne
import numpy as np
...
for pid in ['id001_bt', 'id002_sd', 'id003_mg', 'id004_bj', 'id005_ft']:
    print(pid)
    szr_type = 'interictal'
    szr_index = 0
    ...
    # clinical_hypothesis = True     #TODO change
    for clinical_hypothesis in [True, False]:
        for run_id in [1,2,3]:
            ...
            run = szr_index + run_id # run 01, run 02 and run 03 (szr_index = 0)
            ...
            if np.isnan(correlation):
                correlation = 0
            if np.isnan(correlation_grouped):
