...
replace_signal = True # It seems that 1Hz stimulation doesn't need to be replaced. That sounded romantic.
...
for ich, ch in enumerate(ch_names):
    if peak_vals[ich] > 1: # TODO change
...
for ich, ch in enumerate(ch_names_sim):
    if peak_vals_sim[ich] > 2: # TODO change
...
try:
    t_SO_max_sim = min([onsets_grouped_sim[i] for i in np.where(binary_sim == 1)[0]]) + 3#0.1 # TODO see if needs change
except:
    t_SO_max_sim = 0
...
filepath = Path('~/MyProjects/Epinov_trial/simulate_data/patients_stats_emp_sim_stimulated_control.csv')
