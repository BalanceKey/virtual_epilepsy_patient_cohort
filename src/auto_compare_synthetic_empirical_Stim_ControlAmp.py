...
control datasets against empirical data for different stimulation amplitude
'''
...
database = f'/Users/dollomab/MyProjects/Epinov_trial/simulate_data/patients/BIDS/ControlCohortStimAmplitude/' # TODO change
...
seizure_list = glob.glob(f'{sim_patient_data}/ses-0{ses}/ieeg/{pid_bids}_ses-0{ses}_task-{task}_acq-{acq}_run*_ieeg.vhdr')
for seizure_name in seizure_list:
    run = seizure_name.split('_')[-2]
    stim_parameters_path =  f'{database}/derivatives/tvb/{pid_bids}/ses-0{ses}/{acq}/parameters/{pid_bids}_stimulation_parameters_{run}.tsv'
    stim_parameters = pd.read_csv(stim_parameters_path, sep='\t')
    stim_amplitude =  float(stim_parameters['I'])

    print('ses ' + str(ses) + ', ' + task + ', ' + acq + ', run ' + run + ', stim_amplitude=' + str(stim_amplitude))
    ...
    # replace_signal = True # 1 Hz stimulation doesn't need to be replaced  # TODO change
    if szr_type == 'stimulated' and replace_signal:
        # replace_onset = 1
        # replace_offset = 2
    ...
    binary_emp, binary_sim, overlap = compute_overlap(ez_channels_slp_grouped, ez_channels_slp_grouped_sim, channel_names_grouped, plot=False)
    ...
    try:
        t_SO_max_sim = min([onsets_grouped_sim[i] for i in np.where(binary_sim == 1)[0]]) + 5 # TODO see if needs change
    except:
        t_SO_max_sim = 0
    ...
    # df = pd.DataFrame(columns=['subject_id', 'emp_seizure', 'sim_seizure', 'stim_amplitude',
    ...
    filepath = Path('~/MyProjects/Epinov_trial/simulate_data/patients_stats_emp_sim_stimulated_control_amplitude.csv')
    ...
    new_row = {'subject_id': pid_bids, 'emp_seizure': szr_names[szr_index].strip("' \""),
               ...
               'sim_seizure': os.path.basename(seizure_name), 'stim_amplitude': stim_amplitude,
