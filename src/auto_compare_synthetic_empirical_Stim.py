...
pid = 'id012_fl'#'id009_ba'#'id008_dmc # TODO change
...
for ich, ch in enumerate(ch_names):
    if peak_vals[ich] > 3: # TODO change
...
database = f'/Users/dollomab/MyProjects/Epinov_trial/simulate_data/patients/BIDS/VirtualEpilepticCohortStim/' # TODO change
sim_patient_data = f'{database}/{pid_bids}'
run = szr_index + 1 # TODO change back to what it was I messed it up here
...
if szr_type == 'stimulated' and replace_signal:
    replace_onset = 2
    replace_offset = 3
    raw_sim = vep_prepare_ret.replace_part_of_signal_previous(raw, raw.info, replace_onset, replace_offset,
                                                          ch_names='all')
    replace_onset = 3
    replace_offset = 4
    raw_sim = vep_prepare_ret.replace_part_of_signal_previous(raw_sim, raw.info, replace_onset, replace_offset,
                                                          ch_names='all')
    replace_onset = 4
    replace_offset = 7
    raw = vep_prepare_ret.replace_part_of_signal_previous(raw_sim, raw.info, replace_onset, replace_offset,
                                                      ch_names='all')
    # replace_onset = 0
    # replace_offset = 2.7
    # replace_scale = 0.7
...
start_sim = 3000#5400 #500 TODO change
...
plot_signal(t_sim, y_sim_AC, ch_names_sim, datafeature=slp_sim, seeg_info={'onset':2, 'offset':7}, ez_channels_slp=ez_channels_slp_sim,
            onsets=t_sim[onsets_sim], offsets=t_sim[offsets_sim], scaleplt=0.05, save_path=save_path_sim)
...
t_SO_max_sim = min([onsets_grouped_sim[i] for i in np.where(binary_sim == 1)[0]]) + 11 #0.1 # TODO see if needs change
...
n_comp_sim = min(10, feature_vector_dim(pca_sim, max_variance=0.95))
...
comp = pca.components_
comp_variance = pca.explained_variance_ratio_
n_comp = min(10, feature_vector_dim(pca, max_variance=0.95))
# Compute PCA on simulated timeseries
...
if t_sim[end_sim] < 60:
    ...
    datetime = pd.date_range(start=f'2024-01-30 00:00:00', end=f'2024-01-30 00:{t_sim[end_sim]//60}:{np.round(t_sim[end_sim]%60, 3)}', periods=t_sim[start_sim:end_sim].size)
...
# option 1 : add row at the end of the list; option 2 : modify values of certain row
option = 2 # TODO change if needed
row_index = 15
...
filepath = Path('~/MyProjects/Epinov_trial/simulate_data/patients_stats_emp_sim_stimulated.csv')
