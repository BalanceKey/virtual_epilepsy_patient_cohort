...
pid = 'id015_gjl'#'id010_cmn'#'id009_ba'#'id008_dmc'#'id007_rd'#'id003_mg'  # 'id005_ft'
pid_bids = f'sub-{pid[2:5]}'
subj_proc_dir = f'/Users/dollomab/MyProjects/Epinov_trial/retrospective_patients/{pid}'
save_data = f'/Users/dollomab/MyProjects/Epinov_trial/simulate_data/patients/BIDS/VirtualEpilepticCohortStim/' # TODO change
...
szr_name = f'{subj_proc_dir}/seeg/fif/GJL_crise2_P_090318CBEX_0001.json'
# Load seizure data
...
dt = 0.05
onset = 2 * sfreq
n_sec = 4 # stim duration
stim_length = n_sec * sfreq + onset # n_sec * sfreq + onset
simulation_length = 40* sfreq # n_sec * sfreq
T = 1/freq * sfreq # pulse repetition period [ms]
tau = 1  # pulse duration [ms]
I = 1.5  # pulse intensity [mA]
...
stim_weights = (gain_prior[idx] - gain_prior[idx].min()) / (gain_prior[idx].max() - gain_prior[idx].min())
...
def plot_pattern(pattern_object):
    ...
    plt.subplot(223)
    plt.plot(pattern_object.time.T, pattern_object.temporal_pattern.T, linewidth=0.1)
    plt.title("Time")
    plt.show()
#     plt.subplot(122)
#     plt.imshow(pattern_object(), aspect="auto")
#     plt.colorbar()
#     plt.title("Stimulus")
#     plt.yticks(np.r_[:len(roi)], roi)
#     plt.xlabel("Time")
...
epileptors.r = np.array([0.00002])#np.array([0.000025])
r_val = 0.0006 # 0.0004
...
if clinical_hypothesis:
    ...
    pz = []
    pz2 = []
    pz.append('Left-Temporal-pole')
    pz.append('Right-Thalamus')
    pz.append('Left-Hippocampus-anterior')
    pz.append('Left-F2-rostral')
    pz.append('Right-Hippocampus-posterior')
    pz.append('Right-T1-lateral-posterior')
    pz.append('Right-Gyrus-rectus')
    pz.append('Left-Rhinal-cortex')
    ...
    ezfile = pd.read_excel(ezfile, f'{pid}_i_ez_1')
    ...
    pz2 = [ezroi[iez] for iez in np.where((ezval >= 0.1) & (ezval < 0.3))[0]]

    pz.append('Right-Hippocampus-anterior')
    pz.append('Right-Rhinal-cortex')
    pz.append('Right-T1-lateral-posterior')
    pz.append('Right-Gyrus-rectus')
    pz.append('Left-Rhinal-cortex')
    pz.append('Right-Hippocampus-posterior')
    pz.append('Right-Amygdala')
    pz.append('Right-Temporal-pole')
    pz.append('Left-Temporal-pole')
    pz.append('Right-Thalamus')
    pz.append('Left-Hippocampus-anterior')
    pz.append('Left-F2-rostral')
...
# epileptors.x0[roi.index('Right-Central-sulcus-upper-limb')] = -2.03
# epileptors.x0[roi.index('Right-Precentral-gyrus-upper-limb')] = -2.03
# epileptors.x0[roi.index('Right-Intraparietal-sulcus')] = -2.5
# epileptors.x0[roi.index('Right-Paracentral-lobule')] = -2.5
# epileptors.x0[roi.index('Left-Gyrus-rectus')] = -2.5
# epileptors.x0[roi.index('Left-F3-pars-opercularis')] = -2.5
# epileptors.x0[roi.index('Left-Caudate-nucleus')] = -2.5
epileptors.x0[roi.index('Left-Anterior-cingulate-cortex')] = -2.5
epileptors.x0[roi.index('Right-Anterior-cingulate-cortex')] = -2.5
epileptors.x0[roi.index('Left-Cerebellar-cortex')] = -2.5
epileptors.x0[roi.index('Right-Cerebellar-cortex')] = -2.5

epileptors.threshold[index_ez] = 0.8
epileptors.threshold[index_pz] = 1.5
epileptors.threshold[index_pz2] = 2

# epileptors.n_stim[roi.index(maxval_roi)] = 1
...
if noisy_sim:
    ...
    nsig = np.array([0.0, 0.0, 0., 0.00002, 0.00002, 0., 0.]) # little to no interictal spikes
    # nsig = np.array([0.005, 0.005, 0., 0.0001, 0.0001, 0., 0.])
    #     nsig = np.array([0.005, 0.005, 0., 0.])
...
id = roi.index('Right-Hippocampus-anterior')
# id = roi.index('Left-Cerebellar-cortex')
print(id)
y = tavg[:, :, [id], 0]
print(tavg.shape)
print('Region ' + roi[id])
...
axs[2].axhline(y=sim.model.threshold[id], linewidth=1.5)
axs[2].set_ylabel('m')
...
y = highpass_filter(seeg, sfreq)  # seeg
beta = 1  # the exponent
...
with open(f'{epi_params_file}.tsv', 'wt') as out_file:
    ...
    tsv_writer.writerow([ez, pz+pz2, epileptors.x0, epileptors.threshold, epileptors.Iext, epileptors.Iext2, epileptors.r, epileptors.r2,
...
param_desc = f"{choi.split('-')[0]} -> [+] {choi.split('-')[0][:-1]}{choi[-1]} -> [-] Impulsions biphasiques : {tau} ms | {freq}Hz | {I}mA | Duree : {n_sec}s"
print(param_desc)
